...
def sample_group_groups_df():
    ...
    return pd.DataFrame(
        {"parent_group_id": ["G1", "G2"], "child_group_id": ["G2", "G3"]}
    )
...
def sample_user_groups_df():
    ...
    return pd.DataFrame({"user_id": ["U1", "U2", "U2"], "group_id": ["G1", "G2", "G3"]})
...
def test_role_mapper_init(sample_builtin_groups):
    ...
    assert mapper.role_groups == expected_groups
...
def test_create_role_mappings(sample_builtin_groups, sample_groups_df):
    ...
    mapper = RoleMapper(sample_builtin_groups)
    role_mappings = mapper.create_role_mappings(sample_groups_df)
...
def test_resolve_group_roles(
    sample_builtin_groups, sample_groups_df, sample_group_groups_df
):
    ...
    roles_df = role_mappings["Roles"]
    group_roles_df = role_mappings["Group_Roles"]
    resolved_group_roles_df = mapper.resolve_group_roles(
        roles_df=roles_df,
        group_groups_df=sample_group_groups_df,
        groups_df=sample_groups_df,
        group_roles_df=group_roles_df,
    )

    # Check direct role assignments
    assert not resolved_group_roles_df.empty
    g1_roles = resolved_group_roles_df[resolved_group_roles_df["group_id"] == "G1"]
    assert len(g1_roles) == 1  # G1 (Administrators) has its own role

    # Check inherited roles
    g2_roles = resolved_group_roles_df[resolved_group_roles_df["group_id"] == "G2"]
    assert len(g2_roles) == 2  # G2 has its own role and inherits from G1

    g3_roles = resolved_group_roles_df[resolved_group_roles_df["group_id"] == "G3"]
    assert len(g3_roles) == 3  # G3 has its own role and inherits from G1 and G2


def test_resolve_user_roles(
    sample_builtin_groups,
    sample_groups_df,
    sample_group_groups_df,
    sample_user_groups_df,
):
    ...
    user_roles_df = mapper.resolve_user_roles(
        user_groups_df=sample_user_groups_df, group_roles_df=resolved_group_roles_df
    )
    ...
    assert (
        len(u2_roles) == 3
    )  # U2 is in Users and Exchange Admins groups, plus inherited roles
